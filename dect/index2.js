...
// Additional link analysis using URLScan.io API
const containsLink = /(?:https?:\/\/)?[^\s]+/i.test(message.content); // Capture http, https, and domain-only URLs

let isPhishingLink = false;
if (containsLink) {
  const links = message.content.match(/(?:https?:\/\/)?[^\s]+/gi); // Capture all URLs case-insensitively

  for (const link of links) {
    // Prepend http:// if no protocol is specified
    const fullUrl = link.startsWith('http') ? link : `http://${link}`;

    try {
      console.log(`Submitting URL for scan: ${fullUrl}`);
      const response = await axios.get(`https://urlscan.io/api/v1/scan/`, {
        params: {
          url: fullUrl,
        },
        headers: {
      ...
      // Check for malicious indicators in the response
      if (response.data.verdicts && response.data.verdicts.some(verdict => verdict.category === 'malicious')) {
        isPhishingLink = true;
        break;
      }
    } catch (error) {
